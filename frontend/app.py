...
with st.sidebar:
    ...
    use_rag = st.toggle(
        "Use RAG",
        value=True,
        help="Let the model call the retriever tool when helpful.",
    )
    top_k = st.slider("Top-K documents", 1, 10, 4)
    temperature = st.slider("Temperature", 0.0, 1.0, 0.3, 0.1)
    max_tokens = st.number_input(
        "Max tokens (model output)", min_value=64, max_value=4096, value=512, step=64
    )
                    ...
                    if t == "sources":
                        ...
                        with rag_box.container():
                            ...
                            for i, s in enumerate(sources_buf):
                                with st.expander(
                                    f"Source {i+1}: {s.get('title') or s.get('id')}",
                                    expanded=False,
                                ):
                                    st.code(s.get("text", ""))
                    elif t == "token":
